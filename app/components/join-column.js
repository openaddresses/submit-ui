  ...
  actions: {
    editField: function(heading, index, column){
      ...
      for (var i = 0; i < this.get('numberOfExamples'); i++){
        this.model.submission.get('exampleRows')[i][heading].replace(index, 1, this.model.webServiceResponse.get('source_data').results[i][column])
      }
    },
    addField: function(heading, column){
      ...
      for (var i = 0; i < this.get('numberOfExamples'); i++){
        this.model.submission.get('exampleRows')[i][heading].addObject(this.model.webServiceResponse.get('source_data').results[i][column]);
      }
